...
let exportedMethods = {
    ...
    addBand(bandName, bandMembers, yearFormed, genres, recordLabel) {
        ...
        */
        if (typeof bandName !== "string") throw "No bandName provided";
        if (bandName == undefined || bandName == 'number') throw "Invalid bandName";
        if (bandMembers == undefined || Array.isArray(bandMembers) || bandMembers.length === 0) {
            throw "Invalid bandMembers input";
        }
        if (bandMembers.firstName == undefined || bandMemebers.lastName == undefined || !bandMembers.firstName || !bandMembers.lastName) {
            throw "Invalid "
        }
        if (typeof yearFormed !== 'number' || yearFormed == undefined || yearFormed < 1900 || yearFormed >= 2019) throw "Not a valid yearFormed provided";
        if (genres == undefined || Array.isArray(genres) || genres.length === 0) {
            throw "Invalid genres input";
        }
        if (typeof recordLabel !== "string" || recordLabel == undefined || recordLabel == 'number') throw "Invalid recordLabel provided";
        ...
        const newInsertInfo = await bandCollection.insertOne(newBand);
        return await newInsertInfo;
    ...
    searchBandByName(bandName) {
        ...
        */
        if (!bandName || bandName == undefined || bandName !== 'string') throw 'You must provide a bandName';
        const bandCollection = await bands();
        let regex = new RegExp([".*", bandName, ".*"].join(""), "i");
        let bnd = await bandCollection.find({ 'bandName': regex }).toArray();
        if (bnd.length === 0) throw 'Error'
        return bnd;
    },
    ...
    searchBandByGenre(genre) {
        ...
        */
        if (!genre) throw 'You must provide an array of potentially matching ratings';
        const bandCollection = await bands();
    ...
    searchBandByYear(year) {
        /*  
          ...
          Throw an error if no bands found
        */
        if (year === undefined || year !== 'number') throw 'You must give a starting year';
        const bandCollection = await bands();
        return await bandCollection.find({ 'year': year }).toArray();
    ...
    searchBandFormedBefore(year) {
        ...
        */
        if (year === undefined || year !== 'number') throw 'You must give a starting year';
        const bandCollection = await bands();
        return await bandCollection.find({ 'year': { $lt: year } }).toArray();
    },
    searchBandFormedOnOrBefore(year) {
        ...
        */
        if (year === undefined || year !== 'number') throw 'You must give a starting year';
        const bandCollection = await bands();
        return await bandCollection.find({ 'year': { $lte: year } }).toArray();
    },
    searchBandFormedAfter(year) {
        ...
        */
        if (year === undefined || year !== 'number') throw 'You must give a starting year';
        const bandCollection = await bands();
        return await bandCollection.find({ 'year': { $gt: year } }).toArray();
    },
    searchBandFormedOnOrAfter(year) {
        ...
        */
        if (year === undefined || year !== 'number') throw 'You must give a starting year';
        const bandCollection = await bands();
        return await bandCollection.find({ 'year': { $gte: year } }).toArray();
    },
